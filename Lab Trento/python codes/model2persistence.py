...
def ComputePD(D, vF, max_vF):
    ...
    return [dZero, dOne, dTwo]


def savePD(diagram, filename, fun):

    wfile = open(
        "PDs/PD_{}_{}_H0.txt".format(filename, fun), "w")
    ...
    wfile = open(
        "PDs/PD_{}_{}_H1.txt".format(filename, fun), "w")
    ...
    wfile = open(
        "PDs/PD_{}_{}_H2.txt".format(filename, fun), "w")
...
def visualizePD(diagram, filename, fun):
    ...
    plt.savefig("images/Diagram_{}_{}.png".format(filename, fun))
    plt.clf()
...
createColoredOFF(modname, funname, extremal_values[0][funname-1],extremal_values[1][funname-1])
