...
def computeBottleneckMatrix(fun, d, nummod):
    ...
    for i in range(1, nummod+1):
        diagram = np.loadtxt(
            "PDs/PD_{}_{}_H{}_NoInf.txt".format(i, fun, d))
    ...
    plt.savefig("Distances/bottleneck_matrix_{}_H{}_NoInf.png".format(fun,d))
    plt.clf()


    # ##### SAVING BOTTLENECK DISTANCES ########
    #
    # wfile = open("Distances/bottleneck_Matrix_{}_H{}_NoInf.txt".format(fun,d), "w")
...
def computeCombinedBottleneckMatrix(fun, nummod):
    ...
    plt.savefig("distances/bottleneck_matrix_{}_NoInf.png".format(fun))
    plt.clf()


    # ##### SAVING BOTTLENECK DISTANCES ########
    #
    # wfile = open("distances/bottleneck_Matrix_{}_NoInf.txt".format(fun), "w")
    # np.savetxt(wfile, distances,fmt='%f')
    # wfile.close()
